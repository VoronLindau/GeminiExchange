...
APP_VERSION = "v34.0 (JSON Review & Sync Engine)"
    ...
    for op, i1, i2, j1, j2 in sm.get_opcodes():
        if op == 'equal':
            ...
            for k in range(max(len(objs_l[i1:i2]), len(objs_r[j1:j2]))):
                ...
                c_l = clean_text_for_diff(o_l['text']); c_r = clean_text_for_diff(o_r['text'])
                disp_l = remove_visual_breaks(o_l['text']) if ignore_breaks else o_l['text']
                disp_r = remove_visual_breaks(o_r['text']) if ignore_breaks else o_r['text']
                cid = extract_id(o_r['text']) or extract_id(o_l['text'])
                if cid: 
                ...
                if ignore_breaks and c_l == c_r:
                    if disp_l or disp_r: buckets[current_id].append({'tag': 'equal', 'left': html.escape(disp_l), 'right': html.escape(disp_r), 'page_l': o_l['page'], 'page_r': o_r['page']})
                    continue

                if o_l['text'] and o_r['text']:
                    l_imgs = len(re.findall(r'\[BILD:', o_l['text'])); r_imgs = len(re.findall(r'\[BILD:', o_r['text']))
            ...
            <script>
                ...
                function reloadDiff() {
                    ...
                    fetch('/api/diff_data?ignore_breaks=' + (ignore ? '1' : '0')).then(res => res.json()).then(data => {
                        ...
                        reviewData.metadata = metaData; // Bind latest metadata
                ...
                function buildReviewWidget(reqId) {
                    ...
                    <div class="review-widget">
                        ...
                        <input type="text" placeholder="Kommentar..." value="${html.escape(data.comment)}" onchange="updateReview('${reqId}', 'comment', this.value)" />
                    </div>`;
                }
                // =========================================================
                ...
                function renderSpecialTags(htmlStr, side) {
                    ...
                    res = res.replace(/\[SEITENUMBRUCH:\s*(\d+)\]/g, ``); // Optisch unsichtbar machen!
                    return res;
                }
