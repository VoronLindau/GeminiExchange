...
# und es anschließend in ein spezifisches Modul einzubinden.
# Die Logik wurde in wiederverwendbare Funktionen aufgeteilt.
#
# ********************************************************************************************************************************
...
# =================================================================================================
# HILFSFUNKTIONEN (aus dem Originalskript übernommen)
# =================================================================================================
...
def connect_to_elm(host, username, password, jts_context='jts', rm_context='rm4', caching=2):
    ...
    Args:
        host (str): Die Host-URL des Servers (z.B. 'https://tbd.de').
        username (str): Der Benutzername.
        password (str): Das Passwort.
        jts_context (str): Der Kontext für JTS.
        rm_context (str): Der Kontext für die RM-Anwendung.
        caching (int): Caching-Kontrolle (0=an, 1=leeren, 2=aus).

    Returns:
        elmclient.apps.rm.RM: Das initialisierte RM-Anwendungsobjekt.
    """
...
def create_artifact_in_folder(component, artifact_type, artifact_title, folder_path):
    ...
    Args:
        component (elmclient.apps.rm.Component): Das Komponentenobjekt, in dem gearbeitet wird.
        artifact_type (str): Der Typ des zu erstellenden Artefakts (z.B. "Requirement" oder "Heading").
        artifact_title (str): Der Titel und primäre Text des neuen Artefakts.
        folder_path (str): Der Pfad zum Ordner, in dem das Artefakt erstellt werden soll.

    Returns:
        tuple[str, str]: Eine Tuple mit der URL und der ID des neuen Artefakts.
    """
    ...
    # 2. Creation Factory und Shape für den Artefakttyp finden
    ...
    # 3. XML-Payload für das neue Artefakt erstellen
    # Basis-XML für alle Typen
    ...
    # 4. Artefakt per POST-Request erstellen
    ...
    # 5. ID des neuen Artefakts abrufen und zurückgeben
...
def bind_artifact_to_module_structure(component, module_uri, artifact_to_bind_uri):
    ...
    # 1. Modul-Metadaten abrufen, um die Struktur-URL zu erhalten
    ...
    # 2. ETag und aktuelle Modulstruktur abrufen
    ...
    # 3. Neues Binding-XML erstellen
    # Annahme: Das Binding wird an einer festen Stelle eingefügt (hier: unter dem ersten Kind des Wurzelelements)
    # Für eine flexiblere Lösung müsste hier eine Zielposition gesucht werden.
    ...
    # 4. Aktualisierte Struktur per PUT zurückschreiben
    ...
    # 5. Auf den asynchronen Job warten, falls einer gestartet wurde
...
def main():
    ...
    # Argumente der Kommandozeile verarbeiten
    ...
    try:
        ...
        # 2. Projekt, Komponente und Konfiguration auswählen
        ...
        # 3. Neues Artefakt im Zielordner erstellen
        ...
        # 4. Modul finden, in das eingebunden werden soll
        module_uri = find_module_by_name(component, args.module_name)

        # 5. Artefakt in die Modulstruktur einbinden
        # Wir müssen das Artefakt über seine ID in der aktuellen Konfiguration finden, um die richtige URI für das Binding zu erhalten
        qcbase = component.get_query_capability_uri("oslc_rm:Requirement")
