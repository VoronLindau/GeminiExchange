...
APP_VERSION = "v19.0 (Diff-Driven Semantic Engine)"
...
# ==========================================
# PURE PYTHON XML PARSER (Ohne MS Word)
# ==========================================
    ...
    try:
        with zipfile.ZipFile(file_path, 'r') as z:
            ...
            if 'word/document.xml' in z.namelist():
                ...
                # Extrahiere nackten Text, ignoriere alle Tabellenstrukturen
...
def heal_text(lines):
    """Repariert zerrissene IDs (z.B. '2' und '.1.9' werden im Array zu '2.1.9')"""
    res = []
    last_major = ""
    i = 0
    while i < len(lines):
...
def berechne_diff_daten(lines_left, lines_right):
    ...
    # 1. Der Flat-Diff (Inhalt über Form)
        ...
        <head>
            ...
            <style>
                ...
                @media print { 
                    ...
                    .bg-delete { background-color: #ffe6e6 !important; color: #000; -webkit-print-color-adjust: exact; print-color-adjust: exact;}
                    .bg-insert { background-color: #e6ffe6 !important; color: #000; -webkit-print-color-adjust: exact; print-color-adjust: exact;}
                    .bg-replace { background-color: #e6f2ff !important; color: #000; -webkit-print-color-adjust: exact; print-color-adjust: exact;}
                }
            </style>
        </head>
        <body>
            ...
            <script>
                ...
                window.onload = function() {
                    fetch('/api/diff_data').then(res => res.json()).then(data => {
                        if (data.error) {
                            document.getElementById('loading-screen').innerHTML = `<span style="color:#dc3545">❌ Fehler:</span><br><br><span style="font-size:14px; color:#d4d4d4">${data.error}</span>`;
                            return;
                        }
                ...
                function drawLines() {
                    ...
                    const showEqual = document.getElementById('chk-equal-lines')?.checked;
