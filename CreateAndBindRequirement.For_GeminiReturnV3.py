...
# Version 3: Robuster gegen leere Module und Code-Redundanz entfernt.
#
# Beschreibung:
...
def bind_artifact_to_module_structure(component, module_uri, artifact_to_bind_uri):
    ...
    # --- FIX (Priorität Hoch): Absturz bei leeren Modulen verhindern ---
    # Finde alle möglichen Einfügepositionen
    child_bindings_list = rdfxml.xml_find_elements(modstructure_x, 'rm_modules:Binding/rm_modules:childBindings')
    
    # Prüfe, ob überhaupt eine Position gefunden wurde
    if not child_bindings_list:
        raise ValueError("Konnte keine gültige Einfügeposition in der Modulstruktur finden. Ist das Modul leer oder hat es keine Ordnerstruktur?")
    
    # Greife sicher auf die erste gefundene Position zu
    first_child_bindings = child_bindings_list[0]
...
def main():
    ...
    try:
        ...
        # --- FIX (Priorität Mittel): Redundante Suche entfernt ---
        # Die URI des neuen Artefakts wird direkt verwendet, anstatt es erneut zu suchen.
