...
def berechne_csv_diff(text1, text2, col_l, col_r, delimiter, filters):
    ...
    for i, (raw1, cols1) in enumerate(parsed1):
        if total_p1 > 0 and i % max(1, total_p1 // 50) == 0:
            PROGRESS_STATE = {"status": f"Fuzzy-Analyse ({i}/{total_p1})...", "percent": 30 + int(50 * (i / total_p1))}
            
        if i in matches: continue
        
        key1 = cols1[col_l] if col_l < len(cols1) else ""
        norm_key1 = normalize_text(key1, filters)
        if not norm_key1: continue 
        ...
        for j, (raw2, cols2) in enumerate(parsed2):
            ...
            key2 = cols2[col_r] if col_r < len(cols2) else ""
            norm_key2 = normalize_text(key2, filters)
            if not norm_key2: continue
            
            sm = difflib.SequenceMatcher(None, norm_key1, norm_key2)
    ...
    def get_html_template(self):
        ...
        <head>
            ...
            <style>
                ...
                .code-block { padding: 2px 10px; white-space: pre; border-left: 3px solid transparent; border-right: 3px solid transparent; transition: background-color 0.3s; position: relative; }
                
                .dup-badge { display: inline-block; background: #dc3545; color: white; font-size: 9px; padding: 2px 5px; border-radius: 3px; font-weight: bold; margin-bottom: 4px; border: 1px solid #fff; box-shadow: 0 0 3px rgba(0,0,0,0.5); }
        ...
        <body>
            ...
            <div id="header-container">
                <div class="top-bar">
                    ...
                    <div class="delta-nav-panel">
                        ...
                        <div style="display: flex; gap: 4px; width: 90%; margin-top: 2px;">
                            ...
                            <button id="toggle-dups-btn" onclick="setFilter('dups')" class="filter-btn btn-purple" title="Nur Duplikate (Mehrfache IDs) anzeigen">Dups</button>
                        </div>
                ...
                <div id="csv-toolbar">
                    ...
                    Spalte (Links) <input type="number" id="csv-col-left" value="1" min="1"> 
                    mit Spalte (Rechts) <input type="number" id="csv-col-right" value="1" min="1">
                    
                    <div class="dropdown-check-list" id="filter-dropdown">
                        <span class="anchor" onclick="toggleFilterDropdown()">⚙️ Normalisierung...</span>
                        <ul class="items">
                            <li><input type="checkbox" value="ignore_case" id="f_case"/> <label for="f_case">Aa Groß/Klein ignorieren</label></li>
                            <li><input type="checkbox" value="norm_space" id="f_space" checked/> <label for="f_space">_ Leerzeichen glätten</label></li>
                            <li><input type="checkbox" value="norm_typo" id="f_typo"/> <label for="f_typo">»« Typografie fixen</label></li>
                            <li><input type="checkbox" value="strip_html" id="f_html"/> <label for="f_html">&lt; &gt; HTML-Tags entfernen</label></li>
                        </ul>
                    </div>
            ...
            <script>
                ...
                function toggleFilterDropdown() {
                    document.getElementById('filter-dropdown').classList.toggle('visible');
                }
                ...
                function initDeltas() {
                    deltaBlocks = diffData.filter(b => shouldShow(b.tag, b.ratio, b.dup_left, b.dup_right));
                    currentDeltaIndex = -1;
                ...
                function shouldShow(tag, ratio, dup_l, dup_r) {
                    if (activeFilter === 'all') return true;
                    if (activeFilter === 'dups') return dup_l || dup_r;
                ...
                function applyFilters() {
                    ...
                    document.getElementById('toggle-dups-btn').classList.toggle('toggle-active', activeFilter === 'dups');

                    diffData.forEach(block => {
                        let show = shouldShow(block.tag, block.ratio, block.dup_left, block.dup_right);
                        let displayStyle = show ? 'block' : 'none';
                        let flexStyle = show ? 'flex' : 'none';
                        
                        let lEl = document.getElementById(`left-block-${block.id}`);
                        if (lEl) lEl.style.display = displayStyle;
                        
                        let rEl = document.getElementById(`right-block-${block.id}`);
                        if (rEl) rEl.style.display = displayStyle;
                        
                        let row = document.getElementById(`summary-row-${block.id}`);
                        if (row) row.style.display = flexStyle;
                    });
                ...
                function exportExcel() {
                    ...
                    diffData.forEach(block => {
                        ...
                        if (block.dup_left) txtL = '<b style="color:#dc3545;">[⚠️ DUPLIKAT]</b><br>' + txtL;
                        if (block.dup_right) txtR = '<b style="color:#dc3545;">[⚠️ DUPLIKAT]</b><br>' + txtR;
                ...
                function renderEditors() {
                    ...
                    diffData.forEach(block => {
                        ...
                        const sl = document.createElement('div'); sl.className = `summary-col left`; sl.innerHTML = badgeL + (block.left.length > 0 ? block.left.join('') : '&nbsp;');
                        ...
                        const sr = document.createElement('div'); sr.className = `summary-col`; sr.innerHTML = badgeR + (block.right.length > 0 ? block.right.join('') : '&nbsp;');
                ...
                function drawLines() {
                    ...
                    diffData.forEach(block => {
                        ...
                        if (block.tag === 'equal' && activeFilter !== 'equals' && activeFilter !== '100') return;
                        ...
                        if (block.tag === 'equal') color = '#555555';
                ...
                async function triggerDiffUpdate() {
                    ...
                    let activeFilters = Array.from(document.querySelectorAll('#filter-dropdown input:checked')).map(cb => cb.value);
