...
APP_VERSION = "v10.6 (Word-Jump & Delta-Sync)"
...
def jump_to_word_page(file_path, page_num):
    """Öffnet das Dokument in MS Word und springt exakt zur gewünschten Seite."""
    if not file_path or not os.path.exists(file_path):
        return False
    try:
        import win32com.client
        word = win32com.client.Dispatch("Word.Application")
        word.Visible = True  # Word aktiv ins Bild holen
        
        abs_path = os.path.abspath(file_path)
        doc = None
        
        # Prüfen, ob das Dokument bei diesem Nutzer schon in Word offen ist
...
class DiffRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # NEUER ENDPUNKT FÜR DEN SEITEN-SPRUNG IN WORD
    ...
    def get_html_template(self):
        ...
        <head>
            ...
            <style>
                ...
                /* ================================ */
                /* STYLES FÜR DEN NEUEN SEITENSPRUNG */
                /* ================================ */
                ...
                /* FIX: Seitenumbruch-Linien in der Delta-Ansicht sichtbar halten! */
                .editor-container.hide-equals .tag-equal { display: none !important; }
                
                @supports selector(:has(div)) {
                    /* Wenn der versteckte Block einen Seitenumbruch hat, blenden wir ihn wieder ein... */
                    .editor-container.hide-equals .tag-equal:has(.page-break-line) {
                        display: block !important;
                        font-size: 0 !important; /* ...verstecken aber den reinen Anforderungstext durch Schriftgröße 0 */
                        color: transparent !important;
                        padding: 0 !important; margin: 0 !important; border: none !important;
                    }
                    /* ...und verstecken explizit die Bilder darin. */
                    .editor-container.hide-equals .tag-equal:has(.page-break-line) img {
        ...
        <body>
            ...
            <script>
                ...
                // NEU: API-Call zum Backend für den Word-Seitensprung
                function jumpToPage(side, page) {
                    fetch(`/api/jump?side=${side}&page=${page}`)
                        .then(res => res.json())
                        .then(data => {
                            if(!data.success) {
                                alert("Konnte Word nicht fernsteuern. Ist die Datei am Speicherort verfügbar?");
                            }
                        });
                }
                ...
                function renderSpecialTags(htmlStr, side) {
                    ...
                    // NEU: Der Seitenumbruch erhält jetzt den integrierten Word-Jump Button!
