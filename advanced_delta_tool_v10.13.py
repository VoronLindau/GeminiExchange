...
APP_VERSION = "v10.13 (Async API & Matrix-Zipper)"
...
# ==========================================
# MODUS 2: PURE PYTHON / ID-PARSER MIT MATRIX-ZIPPER
# ==========================================
def get_docx_data(file_path):
    ...
    try:
        with zipfile.ZipFile(file_path, 'r') as z:
            ...
            def traverse(node):
                # 1. Pfusch-Tabellen erkennen und "Reißverschluss"-artig zusammenfügen
                    ...
                    for tr in node.findall('.//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tr'):
                        ...
                        for tc in tr.findall('.//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tc'):
                            ...
                            for p in tc.findall('.//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}p'):
                                txt = extract_p(p).strip()
                                tc_paras.append(txt) # Behält auch leere Absätze für saubere Synchronisation!
                            tc_paras_list.append(tc_paras)
                        
                        if tc_paras_list:
                            max_len = max(len(l) for l in tc_paras_list)
                            for i in range(max_len):
                                row_parts = []
                                for tc_paras in tc_paras_list:
                                    if i < len(tc_paras) and tc_paras[i]:
                                        row_parts.append(tc_paras[i])
                                joined = " ".join(row_parts).strip()
                                if joined:
                                    content.append(joined + "\n")
                    return # Stoppt tiefere Rekursion, Tabelle ist fertig bearbeitet

                # 2. Normale Textabsätze
                if node.tag == '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}p':
                    txt = extract_p(node).strip()
                    if txt:
                        content.append(txt + "\n")
                    return

                # 3. Weiter durch den Baum navigieren
                for child in node:
                    traverse(child)
...
def parse_requirements(lines):
    ...
    for line in lines:
        match = re.search(r'^\s*(ID\s+\d+:|\[REQ-\d+\]|REQ\s+\d+:|(?:\d+\.){1,}\d+|\.\d+(?:\.\d+)+)', line, re.IGNORECASE)
        if match:
            if current_content:
...
class DiffRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # NEU: Asynchrone Datenbeschaffung (Verhindert Browser-Abstürze)
            ...
            <script>
                ...
                window.onload = function() {
                    fetch('/api/diff_data')
                        ...
                        .catch(err => {
                            document.getElementById('loading-screen').innerHTML = `<span style="color:#dc3545">❌ Fataler Browser-Fehler:</span><br><br><span style="font-size:14px; color:#d4d4d4">${err}</span>`;
                        });
                };
