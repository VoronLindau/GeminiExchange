# server.py
# ANFANG
# server.py (V38.22 - UI Toggle & Image Review Fix)
# TEIL 1 von 2
import os
import json
import zipfile
...
# Globale Variable für den schnellen Datei-Dialog
_TK_ROOT = None
...
def jump_to_word_page(side, page_num):
    ...
    try:
        ...
        except Exception as e: print(f"WARNUNG: Sichtbarkeit blockiert: {e}")
        ...
        try:
            if fallback_path and os.path.exists(fallback_path): os.startfile(os.path.abspath(fallback_path))
        except: pass
        return False
...
class DiffRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_url.path == '/api/preflight_status':
            self.send_response(200); self.send_header('Content-type', 'application/json'); self.end_headers()
            self.wfile.write(json.dumps(preflight.PREFLIGHT_RESULTS).encode('utf-8'))
            return
        if parsed_url.path == '/api/browse_file':
            try:
                ...
                if _TK_ROOT is None:
                    _TK_ROOT = tk.Tk()
                    _TK_ROOT.withdraw()
                _TK_ROOT.attributes('-topmost', True); _TK_ROOT.lift(); _TK_ROOT.focus_force()
                ...
                if file_path:
                    ...
                    if side == 'left':
                        config.START_FILE_LEFT = file_path; config.DISPLAY_NAME_LEFT = filename
                    else:
                        config.START_FILE_RIGHT = file_path; config.DISPLAY_NAME_RIGHT = filename
                    resp = {"status": "ok", "path": file_path, "filename": filename}
                else:
        ...
        if parsed_url.path == '/api/diff_data':
            try:
                ...
                if not config.START_FILE_LEFT or not config.START_FILE_RIGHT: raise ValueError("Bitte wählen Sie zuerst beide Dateien aus.")
                ...
                self.send_response(200); self.send_header('Content-type', 'application/json'); self.end_headers()
                self.wfile.write(json.dumps({"diff": diff, "stats": stats, "metadata": metadata}).encode('utf-8'))
            except Exception as e:
                self.send_response(500); self.send_header('Content-type', 'application/json'); self.end_headers()
                self.wfile.write(json.dumps({"error": str(e)}).encode('utf-8'))
            return
        if parsed_url.path == '/api/jump':
            query = parse_qs(parsed_url.query); side = query.get('side', [''])[0]; page = query.get('page', ['1'])[0]
            self.send_response(200); self.send_header('Content-type', 'application/json'); self.end_headers()
            self.wfile.write(json.dumps({"success": jump_to_word_page(side, page)}).encode('utf-8'))
            return
        if parsed_url.path == '/media':
            query = parse_qs(parsed_url.query); side = query.get('side', [''])[0]; file_path_in_zip = query.get('file', [''])[0]
            docx_path = config.START_FILE_LEFT if side == 'left' else config.START_FILE_RIGHT
            if not docx_path or not os.path.exists(docx_path): self.send_response(404); self.end_headers(); return
            try:
                with zipfile.ZipFile(docx_path, 'r') as z:
                    ...
                    self.send_response(200); self.send_header('Content-type', mime.get(ext, 'application/octet-stream')); self.end_headers()
        ...
        if self.path == '/':
            ...
            try:
                html_content = html_content.replace("CHANGE_HISTORY_LOG_DATA", html.escape(config.CHANGE_HISTORY_LOG).replace("\n", "<br>"))
            ...
            html_content = html_content.replace("NAME_LEFT_ESC", html.escape(config.DISPLAY_NAME_LEFT if config.DISPLAY_NAME_LEFT else "Keine Datei (Links)"))
            html_content = html_content.replace("NAME_RIGHT_ESC", html.escape(config.DISPLAY_NAME_RIGHT if config.DISPLAY_NAME_RIGHT else "Keine Datei (Rechts)"))
...
<head>
    ...
    <style>
        ...
        /* Toggle Button für Kopfzeile */
        #header-toggle-btn { position: absolute; top: 0; left: 50%; transform: translateX(-50%); z-index: 999; background: #252526; border: 1px solid #555; border-top: none; color: #888; font-size: 10px; cursor: pointer; border-radius: 0 0 5px 5px; padding: 3px 15px; box-shadow: 0 2px 5px rgba(0,0,0,0.3); transition: color 0.2s;}
        #header-toggle-btn:hover { color: #9cdcfe; }
        ...
        /* Animierte CSS Fortschrittsanzeige */
        #loading-screen { position:absolute; top:0; left:0; width: 100%; height: 100%; background-color: rgba(30, 30, 30, 0.9); z-index: 2000; display: flex; flex-direction: column; align-items: center; justify-content: center;}
        ...
        .progress-bar { width: 100%; height: 14px; background: repeating-linear-gradient(45deg, #007acc, #007acc 10px, #005f9e 10px, #005f9e 20px); background-size: 28px 28px; animation: move-stripes 1s linear infinite; }
...
<body>
    ...
    <div id="info-modal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-header">Delta Tool - Info & Verlauf (V38.22 All-in-One Fix)</div>
        ...
        <div class="center-panel">
            ...
            <div id="stats-panel" style="display:none; width: 100%; background: #1e1e1e; border-radius: 4px; padding: 6px; box-sizing: border-box; text-align: left; margin-bottom: 5px; border: 1px solid #444;">
                ...
                <div style="display:flex; justify-content: space-between; font-size: 9px; line-height: 1.4;">
                    <div style="width: 48%;"><u style="color:#9cdcfe">Links</u><br>Gleich: <span id="l-eq" style="float:right; font-weight:bold;">0</span><br>Geändert: <span id="l-rep" style="float:right; font-weight:bold;">0</span></div>
                    <div style="border-left: 1px solid #444; padding-left: 4px; width: 48%;"><u style="color:#9cdcfe">Rechts</u><br>Gleich: <span id="r-eq" style="float:right; font-weight:bold;">0</span><br>Geändert: <span id="r-rep" style="float:right; font-weight:bold;">0</span></div>
                </div>
    ...
    <div id="workspace" style="visibility: hidden;">
        <button id="header-toggle-btn" onclick="toggleHeader()">⬆ Kopfzeile ausblenden</button>
    ...
    <script>
        ...
        let syncTimer = null; let lastPageL = -1; let lastPageR = -1;
        let currentDeltaIdx = -1; let deltaElements = [];
        ...
        function toggleHeader() {
            ...
            if (header.style.display === 'none') {
                ...
                btn.innerHTML = '⬆ Kopfzeile ausblenden';
            } else {
                header.style.display = 'none';
                btn.innerHTML = '⬇ Kopfzeile einblenden';
            }
        ...
        function browseFile(side) {
            ...
            fetch(`/api/browse_file?side=${side}`).then(res => res.json()).then(data => {
                if (data.status === 'ok') {
                    ...
                    if (!nameL.includes("Keine Datei") && !nameR.includes("Keine Datei")) { reloadDiff(); } 
                    else { document.getElementById('loading-screen').style.display = 'none'; }
                } else { document.getElementById('loading-screen').style.display = 'none'; }
            }).catch(err => { document.getElementById('loading-screen').style.display = 'none'; alert("Fehler: " + err); });
        }

        function checkPreflightStatus() {
            fetch('/api/preflight_status?nocache=' + new Date().getTime()).then(res => res.json()).then(results => {
                const panel = document.getElementById('preflight-status-panel');
                if (panel) { panel.innerHTML = results.user_message; panel.className = results.pywin32_missing ? 'preflight-warn' : 'preflight-ok'; }
        ...
        function reloadDiff() {
            ...
            fetch('/api/diff_data?ignore_breaks=' + (ignore ? '1' : '0')).then(res => res.json()).then(data => {
                if (data.error) { document.getElementById('load-msg-main').innerHTML = `❌ Backend-Fehler`; document.getElementById('load-msg-sub').innerHTML = data.error; return; }
                diffData = data.diff; statsData = data.stats; metaData = data.metadata; reviewData.metadata = metaData;
                
                document.getElementById('loading-screen').style.display = 'none';
                document.getElementById('workspace').style.visibility = 'visible';
                
                safeSetText('name-left', metaData.file_left); safeSetText('name-right', metaData.file_right);
                renderEditors(document.getElementById('left-editor'), document.getElementById('right-editor'));
            }).catch(err => { document.getElementById('load-msg-main').innerHTML = `❌ Netzwerk-Fehler`; document.getElementById('load-msg-sub').innerHTML = err; });
        }
        ...
        function exportJSON() {
            ...
            const dlAnchorElem = document.createElement('a'); dlAnchorElem.setAttribute("href", dataStr); dlAnchorElem.setAttribute("download", "Delta_Review.json"); dlAnchorElem.click();
        }
        ...
        function buildReviewWidget(reqId) {
            ...
            return `<div class="review-widget">
                <select onchange="updateReview('${reqId}', 'status', this.value)">
                    <option value="">-- Status --</option><option value="accepted" ${sAcc}>✅ Accepted</option><option value="not_accepted" ${sNot}>❌ Not accepted</option><option value="refinement" ${sRef}>⚠️ Refinement needed</option><option value="internal" ${sInt}>💬 Internal clarification</option>
                </select>
                <input type="text" placeholder="Kommentar..." value="${escapeHtml(data.comment)}" onchange="updateReview('${reqId}', 'comment', this.value)" />
            </div>`;
        }

        function jumpToPage(side, page) { fetch(`/api/jump?side=${side}&page=${page}`).then(res => res.json()); }

        function getPage(eid) {
            let ed = document.getElementById(eid); let blocks = ed.getElementsByClassName('bucket-block'); let curr = 1;
            let viewCenter = ed.scrollTop + (ed.clientHeight / 2);
            for(let b of blocks) { if(b.offsetTop <= viewCenter) { curr = parseInt(b.getAttribute('data-page') || curr); } else { break; } }
            return curr;
        }
        ...
        function changeView(side) {
            const editor = document.getElementById(side + '-editor'); const val = document.getElementById('view-' + side).value;
        ...
        function renderSpecialTags(htmlStr, side) {
            ...
            let res = htmlStr.replace(/\[BILD:(.*?)\|HASH:(.*?)\]/g, function(match, pathPart, hashPart) {
                ...
                return `<div style="margin-top:10px; margin-bottom:10px;">${badge}<img src="/media?side=${side}&file=${path}" class="${imgClass}" alt="Bild" onload="equalizeHeights()" onerror="this.outerHTML='<div style=\\'color:red; border:1px solid red; padding:5px;\\'>Bild fehlt</div>'"></div>`;
            });
            return res;
        }

        function equalizeHeights() {
            if(!diffData || diffData.length === 0) return;
            diffData.forEach(block => {
                const lEl = document.getElementById(`l-${block.id}`); const rEl = document.getElementById(`r-${block.id}`);
                if(lEl && rEl) {
                    lEl.style.minHeight = '0px'; rEl.style.minHeight = '0px';
                    const maxH = Math.max(lEl.offsetHeight, rEl.offsetHeight);
                    lEl.style.minHeight = maxH + 'px'; rEl.style.minHeight = maxH + 'px';
                }
            });
            drawLines();
        }
        ...
        function renderEditors(leftEditor, rightEditor) {
            ...
            if (statsData.has_reqs || imgDiffs > 0) {
                ...
                let imgDiffEl = document.getElementById('img-diffs'); if(imgDiffEl) imgDiffEl.textContent = imgDiffs;
            }

            leftEditor.innerHTML = ''; rightEditor.innerHTML = '';
            if(diffData && diffData.length > 0) {
                diffData.forEach(block => {
                    const safeReqId = block.req_id ? block.req_id : `[DELTA-${block.id}]`;
                    const needsReview = block.tag !== 'equal' && safeReqId !== "HEADER (Deckblatt/Inhaltsverzeichnis)";
                    const reviewHTML = needsReview ? buildReviewWidget(safeReqId) : "";

                    const lDiv = document.createElement('div');
                    lDiv.id = `l-${block.id}`; lDiv.className = `bucket-block tag-${block.tag}`; lDiv.setAttribute('data-page', block.page_l);
                    let btnL = `<button class='jump-btn' onclick='jumpToPage("left", ${block.page_l})'>🎯 Word S.${block.page_l}</button>`;
                    let headerTextL = block.req_id && block.req_id !== "HEADER (Deckblatt/Inhaltsverzeichnis)" ? `[${block.req_id}]` : (block.tag !== 'equal' ? `${safeReqId}` : "");
                    let prefixL = `<div style='color:#9cdcfe; font-weight:bold; margin-bottom:6px; font-size:14px; border-bottom:1px solid #444; padding-bottom:2px;'><div style='display:flex; justify-content:space-between; align-items:center;'><span>${headerTextL}</span>${btnL}</div>${(needsReview && reviewSide === 'left') ? reviewHTML : ""}</div>`;
                    lDiv.innerHTML = prefixL + renderSpecialTags(block.left, 'left');
                    leftEditor.appendChild(lDiv);

                    const rDiv = document.createElement('div');
                    rDiv.id = `r-${block.id}`; rDiv.className = `bucket-block tag-${block.tag}`; rDiv.setAttribute('data-page', block.page_r);
                    let btnR = `<button class='jump-btn' onclick='jumpToPage("right", ${block.page_r})'>🎯 Word S.${block.page_r}</button>`;
                    let headerTextR = block.req_id && block.req_id !== "HEADER (Deckblatt/Inhaltsverzeichnis)" ? `[${block.req_id}]` : (block.tag !== 'equal' ? `${safeReqId}` : "");
                    let prefixR = `<div style='color:#9cdcfe; font-weight:bold; margin-bottom:6px; font-size:14px; border-bottom:1px solid #444; padding-bottom:2px;'><div style='display:flex; justify-content:space-between; align-items:center;'><span>${headerTextR}</span>${btnR}</div>${(needsReview && reviewSide === 'right') ? reviewHTML : ""}</div>`;
        ...
        function drawLines() {
            const svg = document.getElementById('lines-svg'); const leftEditor = document.getElementById('left-editor'); const rightEditor = document.getElementById('right-editor'); const container = document.getElementById('svg-container');
            if (!svg || !leftEditor || !rightEditor || !container) return;
            while (svg.firstChild) { svg.removeChild(svg.firstChild); }
            const svgWidth = container.clientWidth || 100; const lScroll = leftEditor.scrollTop; const rScroll = rightEditor.scrollTop;
            diffData.forEach(block => {
                if (block.tag === 'equal') return; 
                const lEl = document.getElementById('l-' + block.id); const rEl = document.getElementById('r-' + block.id);
                if (!lEl || !rEl || lEl.offsetParent === null || rEl.offsetParent === null) return;
                const lY = lEl.offsetTop + 18 - lScroll; const rY = rEl.offsetTop + 18 - rScroll;
                let color = '#007bff'; let opacity = '0.8'; let strokeWidth = '2.5';
                if (block.tag === 'insert') { color = '#28a745'; } else if (block.tag === 'delete') { color = '#dc3545'; } else if (block.tag === 'format') { color = '#007bff'; opacity = '0.6'; strokeWidth = '2'; }
                const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
                path.setAttribute('d', `M 0 ${lY} C ${svgWidth/2} ${lY}, ${svgWidth/2} ${rY}, ${svgWidth} ${rY}`);
                path.setAttribute('fill', 'none'); path.setAttribute('stroke', color); path.setAttribute('stroke-width', strokeWidth); path.setAttribute('opacity', opacity);
        ...
        document.getElementById('left-editor')?.addEventListener('scroll', function() { if (!isSyncingLeft) { isSyncingRight = true; const ratio = this.scrollTop / (this.scrollHeight - this.clientHeight); const rEditor = document.getElementById('right-editor'); rEditor.scrollTop = ratio * (rEditor.scrollHeight - rEditor.clientHeight); drawLines(); } isSyncingLeft = false; doLiveSync(); });
        document.getElementById('right-editor')?.addEventListener('scroll', function() { if (!isSyncingRight) { isSyncingLeft = true; const ratio = this.scrollTop / (this.scrollHeight - this.clientHeight); const lEditor = document.getElementById('left-editor'); lEditor.scrollTop = ratio * (lEditor.scrollHeight - lEditor.clientHeight); drawLines(); } isSyncingRight = false; doLiveSync(); });
