...
APP_VERSION = "v10.15 (Auto-Heal & Tab-Fix)"
...
def generate_word_report():
    ...
    for b in diff_data:
        rid = b.get('req_id')
        if not rid or rid == 'HEADER': continue
        if rid not in report_items:
            report_items[rid] = {'status': b['req_status'], 'left_text': "", 'right_text': ""}
        
        report_items[rid]['left_text'] += "".join(b['left'])
        report_items[rid]['right_text'] += "".join(b['right'])
        
    eq = ins = rep = dele = 0
    details = []
    
    for rid, data in report_items.items():
        if data['status'] == 'equal': eq += 1
        elif data['status'] == 'insert': ins += 1; details.append((rid, data))
        elif data['status'] == 'replace': rep += 1; details.append((rid, data))
        elif data['status'] == 'delete': dele += 1; details.append((rid, data))
    ...
    try:
        ...
        for i, (rid, data) in enumerate(details, 1):
            tag_de = {"insert": "NEU", "replace": "GEÄNDERT", "delete": "GELÖSCHT"}[data['status']]
            sel.Font.Bold = True
            sel.TypeText(f"{i}. [{tag_de}] ID/Kapitel: {rid}\n")
            sel.Font.Bold = False
            
            if "[BILD:" in data['left_text'] or "[BILD:" in data['right_text']:
...
# ==========================================
# MODUS 2: PURE PYTHON / ID-PARSER MIT AUTO-HEALING
# ==========================================
            ...
            def extract_p_data(p_node):
                ...
                for node in p_node.iter():
                    if node.tag == '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}lastRenderedPageBreak':
                        ...
                        if node.text:
                            para_text += node.text
                    # FIX 1: Übersetzt Tabulatoren zwingend in Leerzeichen!
                ...
                return para_text, pb_count

            def traverse(node):
                if node.tag == '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tbl':
                    for tr in node.findall('.//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tr'):
                        tc_list = []
                        for tc in tr.findall('.//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}tc'):
                            ...
                            for p in tc.findall('.//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}p'):
                                txt, pbs = extract_p_data(p)
                                tc_paras.append({"text": txt, "pbs": pbs})
                            tc_list.append(tc_paras)
                        
                        if tc_list:
                            max_len = max(len(l) for l in tc_list)
                            for i in range(max_len):
                                row_parts = []
                                row_pbs = 0
                                for col_idx, tc_paras in enumerate(tc_list):
                                    if i < len(tc_paras):
                                        row_parts.append(tc_paras[i]["text"].strip())
                                        if tc_paras[i]["pbs"] > row_pbs:
                                            row_pbs = tc_paras[i]["pbs"]
                                    else:
                                        row_parts.append("")
                                
                                for _ in range(row_pbs):
                                    page_count[0] += 1
                                    content.append(f"\n[SEITENUMBRUCH: {page_count[0]}]\n")
                                    
                                joined = " ".join(p for p in row_parts if p).strip()
                                content.append(joined + "\n")
                    return

                if node.tag == '{http://schemas.openxmlformats.org/wordprocessingml/2006/main}p':
                    txt, pbs = extract_p_data(node)
                    for _ in range(pbs):
                        page_count[0] += 1
                        content.append(f"\n[SEITENUMBRUCH: {page_count[0]}]\n")
...
def parse_requirements(lines):
    ...
    # FIX 2: Der Phantom ID Magnet
    # Zieht komplett abgerissene IDs und zerschnittene Tabellen wieder logisch zusammen.
    for line in lines:
        stripped = line.strip()
        if not stripped: continue
        
        # Erkennt reine IDs (auch zerstörte wie ".1.9" oder nur "2")
        is_orphan = re.match(r'^(?:ID\s+\d+:|\[REQ-\d+\]|REQ\s+\d+:|(?:\d+\.){1,}\d+|\.\d+(?:\.\d+)+|\d+\.|[A-Z]-\d+)$', stripped, re.IGNORECASE)
        
        if is_orphan:
            pending_ids.append(stripped)
        else:
            if pending_ids:
                healed_prefix = ""
                for pid in pending_ids:
                    # Heilt ".1.9" zu "2.1.9" falls vorher ein "2" erkannt wurde
                    if pid.startswith('.'):
                        healed_prefix += (last_major + pid).replace('..', '.') + " "
                    else:
                        m = re.match(r'^(\d+)', pid)
                        if m: last_major = m.group(1)
                        healed_prefix += pid + " "
                        
                clean_lines.append(healed_prefix.strip() + " " + stripped + "\n")
                pending_ids = []
            else:
                clean_lines.append(stripped + "\n")
                
    # Verbleibende Orphans am Dokumentende abfertigen
    for pid in pending_ids:
        if pid.startswith('.'): pid = (last_major + pid).replace('..', '.')
        else:
            m = re.match(r'^(\d+)', pid)
            if m: last_major = m.group(1)
        clean_lines.append(pid + "\n")
    ...
    for line in clean_lines:
        match = re.search(r'^\s*(ID\s+\d+:|\[REQ-\d+\]|REQ\s+\d+:|(?:\d+\.){1,}\d+)', line, re.IGNORECASE)
        if match:
            if current_content:
...
def berechne_diff_daten(text1, text2):
    ...
    for l_id in left_ids:
        ...
        if l_id in right_map:
            ...
            if str_l == str_r:
                diff_data.append({'id': block_id, 'tag': 'equal', 'req_status': 'equal', 'left': [html.escape(l) for l in l_content], 'right': [html.escape(l) for l in r_content], 'req_id': l_id})
                block_id += 1
            else:
                sm2 = difflib.SequenceMatcher(None, l_content, r_content)
                for op, i1, i2, j1, j2 in sm2.get_opcodes():
                    chunk_l = l_content[i1:i2]
                    chunk_r = r_content[j1:j2]
                    
                    if op == 'equal':
                        diff_data.append({'id': block_id, 'tag': 'equal', 'req_status': 'replace', 'left': [html.escape(l) for l in chunk_l], 'right': [html.escape(l) for l in chunk_r], 'req_id': l_id})
                    elif op == 'replace':
                        str_chunk_l = "".join(chunk_l)
                        str_chunk_r = "".join(chunk_r)
                        
                        if len(str_chunk_l) > 3000 or len(str_chunk_r) > 3000:
                            left_out = f"<span class='char-diff'>{html.escape(str_chunk_l)}</span>"
                            right_out = f"<span class='char-diff'>{html.escape(str_chunk_r)}</span>"
                        else:
                            sm3 = difflib.SequenceMatcher(None, str_chunk_l, str_chunk_r)
                            left_out, right_out = "", ""
                            for op3, k1, k2, m1, m2 in sm3.get_opcodes():
                                part_l = html.escape(str_chunk_l[k1:k2])
                                part_r = html.escape(str_chunk_r[m1:m2])
                                if op3 == 'equal':
                                    left_out += part_l; right_out += part_r
                                else:
                                    if part_l: left_out += f"<span class='char-diff'>{part_l}</span>"
                                    if part_r: right_out += f"<span class='char-diff'>{part_r}</span>"
                                    
                        diff_data.append({'id': block_id, 'tag': 'replace', 'req_status': 'replace', 'left': [left_out], 'right': [right_out], 'req_id': l_id})
                    elif op == 'insert':
                        diff_data.append({'id': block_id, 'tag': 'insert', 'req_status': 'replace', 'left': [], 'right': [html.escape(l) for l in chunk_r], 'req_id': l_id})
                    elif op == 'delete':
                        diff_data.append({'id': block_id, 'tag': 'delete', 'req_status': 'replace', 'left': [html.escape(l) for l in chunk_l], 'right': [], 'req_id': l_id})
                    
                    block_id += 1
            
            del right_map[l_id]
        else:
            diff_data.append({'id': block_id, 'tag': 'delete', 'req_status': 'delete', 'left': [html.escape(l) for l in l_content], 'right': [], 'req_id': l_id})
            block_id += 1
        
    for r_id in right_ids:
        if r_id in right_map:
            diff_data.append({'id': block_id, 'tag': 'insert', 'req_status': 'insert', 'left': [], 'right': [html.escape(l) for l in right_map[r_id]], 'req_id': r_id})
            block_id += 1
    ...
    def do_GET(self):
        ...
        if parsed_url.path == '/api/diff_data':
            try:
                info_l = get_file_info(START_FILE_LEFT)
                info_r = get_file_info(START_FILE_RIGHT)
                diff = berechne_diff_daten(info_l['content'], info_r['content'])
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps({"diff": diff}).encode('utf-8'))
            except Exception as e:
    ...
    def get_html_template(self):
        ...
        <head>
            ...
            <style>
                ...
                .code-block { padding: 2px 5px; white-space: pre-wrap; border-radius: 3px; margin-bottom: 5px; min-height: 1.5em;}
        ...
        <body>
            ...
            <script>
                ...
                function updateStats() {
                    let l_eq = 0, l_rep = 0, l_del = 0;
                    let r_eq = 0, r_rep = 0, r_ins = 0;
                    
                    let pL = new Set();
                    let pR = new Set();

                    diffData.forEach(block => {
                        let reqId = block.req_id;
                        if (reqId && reqId !== 'HEADER') {
                            let status = block.req_status;
                            if (!pL.has(reqId)) {
                                if (status === 'equal') l_eq++;
                                else if (status === 'replace') l_rep++;
                                else if (status === 'delete') l_del++;
                                if (status !== 'insert') pL.add(reqId);
                            }
                            if (!pR.has(reqId)) {
                                if (status === 'equal') r_eq++;
                                else if (status === 'replace') r_rep++;
                                else if (status === 'insert') r_ins++;
                                if (status !== 'delete') pR.add(reqId);
                            }
                        }
                    });

                    document.getElementById('l-eq').textContent = l_eq;
                    document.getElementById('l-rep').textContent = l_rep;
                    document.getElementById('l-del').textContent = l_del;
                    document.getElementById('l-tot').textContent = l_eq + l_rep + l_del;

                    document.getElementById('r-eq').textContent = r_eq;
                    document.getElementById('r-rep').textContent = r_rep;
                    document.getElementById('r-ins').textContent = r_ins;
                    document.getElementById('r-tot').textContent = r_eq + r_rep + r_ins;
                }
